...
function LibraryCard({ item }: { item: MediaItemInfo }) {
  ...
  const isVideo =
    extLower &&
    [".mp4", ".mov", ".mkv", ".avi", ".webm", ".m4v", ".3gp"].includes(
      extLower,
    );
  const isAudio =
    extLower &&
    [".mp3", ".flac", ".wav", ".aac", ".ogg", ".m4a", ".wma"].includes(
      extLower,
    );
    ...
    >
      ...
      <div
        className={cn(
          ...
          isImage && !thumbUrl && !isFailed
            ? "bg-blue-50 dark:bg-blue-950"
            : "",
          isVideo && !thumbUrl && !isFailed
...
export default function LibraryPage() {
  ...
  const lastRegeneratedSessionId = useTransferStore(
    (s) => s.ui.lastRegeneratedSessionId,
  );
  const setLastRegeneratedSessionId = useTransferStore(
    (s) => s.setLastRegeneratedSessionId,
  );
  ...
  useEffect(() => {
    if (lastSessionId != null && lastSessionId !== lastRegeneratedSessionId) {
  ...
  const sessionFilter =
    showingRecent && lastSessionId ? lastSessionId : undefined;
