...
function backoffWithJitter(attempt: number): number {
  const exp = Math.min(WS_RECONNECT_CAP, WS_RECONNECT_BASE * 2 ** attempt);
...
export function useTransferWs(sessionId: number | null) {
  ...
  const connect = useCallback(() => {
    ...
    if (
      TERMINAL_STATUSES.has(sessionStatusRef.current) ||
    ...
    heartbeatTimer.current = setTimeout(() => {
      if (Date.now() - lastMsgAt.current > WS_HEARTBEAT_TIMEOUT) {
        try {
          ws.close();
        } catch {
          // ignore
    ...
    ws.onclose = (event) => {
      ...
      if (event.code === 1008 || event.code === 4003 || event.code === 4403) {
        ...
        setWsError(
          event.code === 4403
            ? "Live updates unavailable (not authenticated)"
            : "Live updates unavailable (session not found)",
        );
        return;
      }
      ...
      if (
        reconnectCount.current < WS_MAX_RECONNECT &&
        shouldReconnectRef.current
      ) {
        ...
        setWsError("Live updates unavailable — polling continues");
      }
    };
