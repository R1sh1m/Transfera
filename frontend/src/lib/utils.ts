...
// absolute display by the UTC offset ("5h ago" for a just-finished backup
// in IST). Parse naive backend strings as UTC; pass through anything that
// already carries a designator. Differences between two backend timestamps
// are unaffected either way, so only absolute displays need this.
const HAS_TZ = /(z|[+-]\d{2}:?\d{2})$/i;
export function parseBackendDate(iso: string | null | undefined): Date {
  if (!iso) return new Date(NaN);
  return new Date(HAS_TZ.test(iso.trim()) ? iso : iso.trim() + "Z");
}
